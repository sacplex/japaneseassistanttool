...
public class HiraganaCanvas extends Canvas implements Runnable
	{
	private Kana kana;
	private Text text;
	...
	private static int TOTAL_NUMBER_OF_CHARACTERS;
	...
	public HiraganaCanvas(Kana kana, long period, int totalCharacters)
		{
		this.period = period;
	...
	private void initHiraganaPanel(Kana kana)
		...
		hiraganaImageLoader = new HiraganaImageLoader("hiragana");
		
		random = new Random();
		
		characterIndexes = new ArrayList<>();
		
		for(int i=0; i<hiraganaImageLoader.getNumberOfCharacters(); i++)
		...
		characterImage = hiraganaImageLoader.getCharacters().get(characterIndexes.get(randomIndex)).getImage();
		character = hiraganaImageLoader.getCharacters().get(characterIndexes.get(randomIndex)).getCharacter();
		characterIndex = hiraganaImageLoader.getCharacters().get(characterIndexes.get(randomIndex)).getIndex();
		
		System.out.println("Audio Character: " + character);
		
		hiraganaAudioLoader = new HiraganaAudioLoader(character);
	...
	public void initHiraganaSubPanel(Kana kana, JFrame frame)
		{
		HiraganaSubCanvas hiraganaSubCanvas = new HiraganaSubCanvas(this, period);
		hiraganaSubCanvas.add(hiraganaAudioLoader);
		
		this.frame = new JFrame();
		this.frame.setResizable(false);
		this.frame.setTitle("Hiragana Input");
		this.frame.add(hiraganaSubCanvas);
		this.frame.pack();
	...
	public void initHiraganaRecallPanel(Kana kana, JFrame frame)
		{
		HiraganaRecallPanel hiraganaRecallPanel = new HiraganaRecallPanel(this);
		hiraganaRecallPanel.add(hiraganaAudioLoader);
		
		this.frame = new JFrame();
		this.frame.setResizable(false);
		this.frame.setTitle("Hiragana Input");
		this.frame.add(hiraganaRecallPanel);
		this.frame.pack();
	...
	public void nextCharacter()
		...
		hiraganaAudioLoader.setNewMedia(character);
		}
	...
	private void drawCharacter()
		...
		g2d.drawString(numberOfCorrectAnswer + "/" + counter + "/" + hiraganaImageLoader.getNumberOfCharacters(), 245, 280);
	...
	public HiraganaImageLoader getHiraganaImageLoader()
		{
		return hiraganaImageLoader;
		}
