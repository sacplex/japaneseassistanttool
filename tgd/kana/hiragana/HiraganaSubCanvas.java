...
public class HiraganaSubCanvas extends Canvas implements Runnable
	{
	...
	public HiraganaSubCanvas(HiraganaCanvas hiraganaCanvas, long period)
		{
		this.hiraganaCanvas = hiraganaCanvas;
	...
	private void initHiraganaIconLoader()
		...
		for(int i=0; i<hiraganaCanvas.getHiraganaImageLoader().getNumberOfCharacters(); i++)
			characterIndexes.add(i);
		
		random = new Random();
		
		for(int i=0; i<hiraganaCanvas.getHiraganaImageLoader().getNumberOfCharacters()-1; i++)
			{
			int randomIndex = random.nextInt(hiraganaCanvas.getHiraganaImageLoader().getNumberOfCharacters()-1);
		...
		for(int i=0; i<numberOfOptions; i++)
			{
			iconOptions.add(hiraganaIconLoader.getCharacters().get(characterIndexes.get(i)));
			}
		...
		for(int i=0; i<numberOfOptions; i++)
			{
			if(hiraganaCanvas.getCharacter().equals(iconOptions.get(i).getCharacter()))
				{
		...
		if(removeAndReplace)
			...
			iconOptions.add(correctOption, hiraganaIconLoader.getCharacters().get(hiraganaCanvas.getCharacterIndex()));
			}
	...
	private void checkAnswer(int selectedOption)
		{
		if(iconOptions.get(selectedOption).getCharacter().equals(hiraganaCanvas.getCharacter()))
			{
